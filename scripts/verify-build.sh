...
# --quiet: suppress per-check details; show only pass/fail
QUIET=0
for arg in "$@"; do [[ "$arg" == "--quiet" ]] && QUIET=1; done

FAILED=0

[[ $QUIET -eq 0 ]] && { echo -e "${GREEN}[*] Verifying fuzzing setup${NC}"; echo ""; }

# Check Poly/ML binary exists
[[ $QUIET -eq 0 ]] && echo -e "${YELLOW}[1/6] Checking Poly/ML binary...${NC}"
if [ -f "$POLY_BIN" ]; then
    [[ $QUIET -eq 0 ]] && echo -e "${GREEN}  [ok] Poly/ML found at: $POLY_BIN${NC}"
else
...
[[ $QUIET -eq 0 ]] && echo -e "${YELLOW}[2/6] Checking harness binary...${NC}"
if [ -f "$HARNESS" ]; then
    [[ $QUIET -eq 0 ]] && echo -e "${GREEN}  [ok] Harness found at: $HARNESS${NC}"
else
    [[ $QUIET -eq 0 ]] && echo -e "${YELLOW}  [!] Harness not found (non-critical: not used in campaigns)${NC}"
fi
...
[[ $QUIET -eq 0 ]] && echo -e "${YELLOW}[3/6] Checking AFL++ instrumentation in poly binary...${NC}"
POLY_INSTRUMENTED=0
...
if "$STRINGS_BIN" "$POLY_BIN" 2>/dev/null | grep -qF "__afl_area_ptr"; then
    ...
    POLY_INSTRUMENTED=1
    [[ $QUIET -eq 0 ]] && echo -e "${YELLOW}  [!] Symbol checks inconclusive; binary executes correctly.${NC}"
    [[ $QUIET -eq 0 ]] && echo -e "${YELLOW}      Confirm edges > 0 after the first campaign run.${NC}"
fi

if [ "$POLY_INSTRUMENTED" -eq 1 ]; then
    [[ $QUIET -eq 0 ]] && echo -e "${GREEN}  [ok] AFL++ instrumentation confirmed in poly binary${NC}"
else
...
[[ $QUIET -eq 0 ]] && echo -e "${YELLOW}[4/6] Checking AFL++ instrumentation in harness...${NC}"
if [[ $QUIET -eq 0 ]]; then
...
[[ $QUIET -eq 0 ]] && echo -e "${YELLOW}[5/6] Checking sanitiser instrumentation...${NC}"
if [[ $QUIET -eq 0 ]]; then
...
[[ $QUIET -eq 0 ]] && echo -e "${YELLOW}[6/6] Testing with regression seed (should trigger UBSan)...${NC}"
if [[ $QUIET -eq 0 ]]; then
...
if [ $FAILED -eq 0 ]; then
    if [[ $QUIET -eq 1 ]]; then
        echo -e "${GREEN}  [ok] Build verified${NC}"
    else
        echo -e "${GREEN}+============================================+${NC}"
