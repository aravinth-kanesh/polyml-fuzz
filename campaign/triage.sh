...
if [ -d "$MINIMISED_DIR" ] && [ "$(ls -A "$MINIMISED_DIR" 2>/dev/null)" ]; then
    ...
    [[ $QUIET -eq 0 ]] && echo -e "${GREEN}[*] Using minimised crashes: $MINIMISED_DIR${NC}"
elif [ -d "$COLLECTED_DIR" ] && [ "$(ls -A "$COLLECTED_DIR" 2>/dev/null)" ]; then
    CRASHES_DIR="$COLLECTED_DIR"
    [[ $QUIET -eq 0 ]] && echo -e "${YELLOW}[!] minimised/ not found; falling back to raw collected crashes${NC}"
else
    echo -e "${RED}[!] No crashes found. Run collect-crashes.sh first${NC}"
    exit 1
fi

[[ $QUIET -eq 0 ]] && echo -e "${GREEN}[*] Triaging crashes from: $CAMPAIGN_NAME${NC}"
...
for crash_file in "$CRASHES_DIR"/*; do
    ...
    [[ $QUIET -eq 0 ]] && echo -e "${BLUE}[$CRASH_COUNT] Reproducing: $CRASH_NAME${NC}"
    ...
    timeout 10 "$POLY_BIN" < "$crash_file" > /dev/null 2> "$LOG_FILE" || {
        ...
        if grep -q "runtime error" "$LOG_FILE"; then
            ...
            [[ $QUIET -eq 0 ]] && echo -e "${YELLOW}    -> UBSan: Undefined behaviour detected${NC}"
        elif grep -q "heap-buffer-overflow" "$LOG_FILE"; then
            FAULT_TYPE="HeapBufferOverflow"
            [[ $QUIET -eq 0 ]] && echo -e "${RED}    -> ASan: Heap buffer overflow${NC}"
        elif grep -q "stack-buffer-overflow" "$LOG_FILE"; then
            FAULT_TYPE="StackBufferOverflow"
            [[ $QUIET -eq 0 ]] && echo -e "${RED}    -> ASan: Stack buffer overflow${NC}"
        elif grep -q "use-after-free" "$LOG_FILE"; then
            FAULT_TYPE="UseAfterFree"
            [[ $QUIET -eq 0 ]] && echo -e "${RED}    -> ASan: Use-after-free${NC}"
        elif [ $EXIT_CODE -eq 124 ]; then
            FAULT_TYPE="Timeout"
            [[ $QUIET -eq 0 ]] && echo -e "${YELLOW}    -> Timeout (possible hang)${NC}"
        elif [ $EXIT_CODE -gt 128 ]; then
            SIGNAL=$((EXIT_CODE - 128))
            FAULT_TYPE="Signal${SIGNAL}"
            [[ $QUIET -eq 0 ]] && echo -e "${RED}    -> Crash: signal $SIGNAL${NC}"
        else
            FAULT_TYPE="Unknown"
            [[ $QUIET -eq 0 ]] && echo -e "${YELLOW}    -> Unknown error (exit code $EXIT_CODE)${NC}"
        fi
...
if [[ $QUIET -eq 1 ]]; then
    if [[ $CRASH_COUNT -eq 0 ]]; then
        echo -e "${GREEN}  [ok] Triage: 0 crashes${NC}"
    else
        echo -e "${GREEN}  [ok] Triage: ${CRASH_COUNT} crashes classified${NC}"
        for fault_type in "${!CRASH_TYPES[@]}"; do
            echo -e "${GREEN}       ${fault_type}: ${CRASH_TYPES[$fault_type]}${NC}"
        done
    fi
else
    echo -e "${GREEN}+============================================+${NC}"
